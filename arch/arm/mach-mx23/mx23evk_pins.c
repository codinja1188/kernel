...
#endif
#if defined(CONFIG_INPUT_FPC1080) || defined(CONFIG_INPUT_FPC1080_MODULE)
static struct pin_desc fpc1080_input_pins[] = {
         {
        .name   = "SSP1_DATA1",         // IRQ
        .id     = PINID_SSP1_DATA1,
        .fun    = PIN_GPIO,
...
#if defined(CONFIG_TOUCHSCREEN_MSG2133) || defined(CONFIG_TOUCHSCREEN_MSG2133_MODULE)
static struct pin_desc msg2133_ts_pins[] = {
         {
        .name   = "SSP1_DATA1",         // IRQ
        .id     = PINID_SSP1_DATA1,
        .fun    = PIN_GPIO,
...
#endif
#if defined(CONFIG_INPUT_FPC1080) || defined(CONFIG_INPUT_FPC1080_MODULE)
int  mxs_spi_fpc1080_pin_init(void)
{
	mxs_request_pins(mx23evk_spi_pins, ARRAY_SIZE(mx23evk_spi_pins));	
	mxs_request_pins(fpc1080_input_pins, ARRAY_SIZE(fpc1080_input_pins));
	return 0;
}
int  mxs_spi_fpc1080_pin_release(void)
{
	mxs_release_pins(mx23evk_spi_pins, ARRAY_SIZE(mx23evk_spi_pins));	
	mxs_release_pins(fpc1080_input_pins, ARRAY_SIZE(fpc1080_input_pins));	
...
int  mxs_spi_fpc1080_pin_init(void)
{
	...
	int error;	
//	pdata->irq_gpio = MXS_PIN_TO_GPIO(PINID_SSP1_DATA1);
//	pdata->reset_gpio = MXS_PIN_TO_GPIO(PINID_SSP1_DATA2);
	pdata->irq_gpio = 67;
	pdata->reset_gpio = 68;

	error = gpio_request(pdata->reset_gpio, "fpc1080_reset");
	if (error) {
                dev_err(&pdata->reset_gpio,
	...
	error = gpio_request(pdata->irq_gpio, "fpc1080 irq");
	if (error) {
                dev_err(&pdata->irq_gpio, "gpio_request (irq) failed.\n");
        }

	error =	gpio_direction_input(pdata->irq_gpio);
	if (error) {
                dev_err(&pdata->irq_gpio,
...
//	set_irq_type(gpio_to_irq(pdata->reset_gpio), IRQ_TYPE_NONE);
