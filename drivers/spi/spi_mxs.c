...
static int mxs_spi_init_hw(struct mxs_spi *ss)
{
	...
	ss->clk = clk_get(NULL, "ssp.0"); // original
				  ...
				  struct spi_transfer *t)
{
	...
	printk(KERN_ERR"********%s:%d 2:write Hardware \n", __FUNCTION__,__LINE__);
	...
	/******* changes done by vasubabu ******/
	/*int ret;
	ret = __raw_readl(ss->regs + HW_SSP_CTRL1);	
	printk(KERN_ERR"*******value:%d %s:%d 3:Enter\n", ret, __FUNCTION__,__LINE__);*/
	/******* changes done by vasubabu ******/
...
/*****************************************************************************
******* changes done by vasubabu*********************************************
just comment the macros for pdata dependency in spi_mxs.c file (spi bus driver)
********************************************************************************
*/
#if defined(CONFIG_ENC28J60) || defined(CONFIG_ENC28J60_MODULE)
			    ...
			    int *first, int *last, int write)
{
	...
	printk(KERN_ERR "************************** Dma map single %s:%d 2:Enter \n", __FUNCTION__, __LINE__);
	if (!dma_buf)
...
static int mxs_spi_handle_message(struct mxs_spi *ss, struct spi_message *m)
{
	...
	printk(KERN_ERR"********%s:%d 2: Enter \n", __FUNCTION__,__LINE__);
		 ...
		 */
		if (t->tx_buf) {
			printk(KERN_ERR"********%s:%d 3: Enter tx \n", __FUNCTION__,__LINE__);
			status = pio ?
			    ...
			    mxs_spi_txrx_dma(ss, cs, (void *)t->tx_buf,
					     ...
					     1);
			if (debug) {
				printk(KERN_ERR"********%s:%d 4: Enter debug \n", __FUNCTION__,__LINE__);
				if (t->len < 0x10)
		...
		if (t->rx_buf) {
			...
			printk(KERN_ERR"********%s:%d 5: Enter rx \n", __FUNCTION__,__LINE__);
			status = pio ?
			    ...
			    mxs_spi_txrx_dma(ss, cs, t->rx_buf,
					     ...
					     0);
			if (debug) {
				printk(KERN_ERR"********%s:%d 6: Enter debug \n", __FUNCTION__,__LINE__);
				if (t->len < 0x10)
...
static void mxs_spi_handle(struct work_struct *w)
{
	...
	while (!list_empty(&ss->queue)) {
		...
		printk(KERN_ERR"**********%s:%d 2:In while loop\n", __FUNCTION__,__LINE__);
