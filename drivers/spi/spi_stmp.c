...
{
	int err;
	printk(KERN_ERR " stmp_spi_init_hw() started ...\n");
	...
	printk(KERN_ERR " stmp_spi_init_hw() ended ...\n");
	return 0;
...
{
	printk(KERN_ERR "SPI: stmp_spi_release_hw() started ...\n");
	if (ss->clk && !IS_ERR(ss->clk)) {
		clk_disable(ss->clk);
		clk_put(ss->clk);
	}
	stmp37xx_spi_pins_release((char *)dev_name(ss->master_dev), ss->id);
	printk(KERN_ERR "SPI: stmp_spi_release_hw() ended ...\n");
}
		...
		struct spi_transfer *t)
{
	...
	u16 rate;
	printk(KERN_ERR "SPI: stmp_spi_setuo_transfer() started ...\n");
	...
	printk(KERN_ERR "SPI: stmp_spi_setuo_transfer() ended ...\n");
	return 0;
}
...
static void stmp_spi_cleanup(struct spi_device *spi)
{
	...
	printk(KERN_ERR "SPI: stmp_spi_cleanup() started ...\n");
	if (pdata && pdata->hw_release)
		pdata->hw_release(spi);
	if (pdata && pdata->hw_release)
	printk(KERN_ERR "SPI: stmp_spi_cleanup() ended ...\n");
}
...
static int stmp_spi_setup(struct spi_device *spi)
{
	...
	struct stmp_spi *ss;
	int err = 0;
	printk(KERN_ERR "SPI:stmp_spi_setup() started ...\n");
	ss = spi_master_get_devdata(spi->master);
	...
	if (err)
		goto out2;
	printk(KERN_ERR "SPI:stmp_spi_setup() ended ...\n");
	return 0;
...
{
	int count;
	printk(KERN_ERR "SPI:stmp_spi_txrx_pio() started ...\n");
	...
	printk(KERN_ERR "SPI:stmp_spi_txrx_pio() ended ...\n");
...
static int stmp_spi_handle_message(struct stmp_spi *ss, struct spi_message *m)
{
	...
	first = last = 0;
	printk(KERN_ERR "SPI:stmp_spi_handle_message() started ...\n");
	...
	printk(KERN_ERR "SPI:stmp_spi_handle_message() ended ...\n");
	return status;
}
...
static void stmp_spi_handle(struct work_struct *w)
{
	...
	printk(KERN_ERR "SPI:stmp_spi_handle() started ...\n");
	BUG_ON(w == NULL);
	...
	printk(KERN_ERR "SPI:stmp_spi_handle() ended ...\n");
	return;
}
...
static int stmp_spi_transfer(struct spi_device *spi, struct spi_message *m)
{
	...
	printk(KERN_ERR "SPI:stmp_spi_transfer() started ...\n");
	m->status = -EINPROGRESS;
	spin_lock_irqsave(&ss->lock, flags);
	list_add_tail(&m->queue, &ss->queue);
	queue_work(ss->workqueue, &ss->work);
	spin_unlock_irqrestore(&ss->lock, flags);
	printk(KERN_ERR "SPI:stmp_spi_transfer() ended ...\n");
	return 0;
}
...
static irqreturn_t stmp_spi_irq(int irq, void *dev_id)
{
	...
	printk(KERN_ERR "SPI:stmp_spi_irq() started ...\n");
	stmp3xxx_dma_clear_interrupt(ss->dma);
	complete(&ss->done);
	printk(KERN_ERR "SPI:stmp_spi_irq() ended ...\n");
	return IRQ_HANDLED;
}
...
static int __init stmp_spi_probe(struct platform_device *dev)
{
	...
	struct resource *r;
	u32 mem;
	printk(KERN_ERR "SPI:stmp_spi_probe() started..\n");
	printk(KERN_ERR "SPI: spi_master allocated...  \n");
	...
	printk(KERN_ERR "SPI: getting platform data started ....\n");
	...
	ss->id = dev->id;
	printk(KERN_ERR "SPI: spi master methods started...\n");
	...
	printk(KERN_ERR "SPI: Registering irq started ...\n");
	...
	printk(KERN_ERR " SPI: Registering master device ...\n");
	...
	printk(KERN_ERR " SPI: probe ended...\n");
	return 0;
...
static int __devexit stmp_spi_remove(struct platform_device *dev)
{
	struct stmp_spi *ss;
	struct spi_master *master;
	printk(KERN_ERR "SPI: stmp_spi_remove() started ...\n");
	...
	printk(KERN_ERR "SPI: stmp_spi_remove() ended ...\n");
out1:
	spi_master_put(master);
out0:
	return 0;
