...
#define ADC_SETUP_MODE (S_IWUSR|S_IWGRP|S_IWOTH|S_IRUSR|S_IRGRP|S_IROTH)
				...
				struct device_attribute *attr, char *buf)
{
	...
	if (fpc_attr->offset == offsetof(struct fpc1080_adc_setup, gain))
		adc_val = fpc1080->adc_setup.gain;

	if (fpc_attr->offset == offsetof(struct fpc1080_adc_setup, offset))
		adc_val = fpc1080->adc_setup.offset;

	if (fpc_attr->offset == offsetof(struct fpc1080_adc_setup, pxl_setup))
		adc_val = fpc1080->adc_setup.pxl_setup;
				...
				struct device_attribute *attr, char *buf)
{
	...
	if (fpc_attr->offset ==	offsetof(struct fpc1080_diag,
					 frames_captured)) {
...
{
//	printk(KERN_ERR"*****:%s:%d write register started \n",__FUNCTION__,__LINE__);
	int error;
...
//	printk(KERN_ERR"*****%s:%d addr:%x, value:%x \n", __FUNCTION__,__LINE__,fpc1080->huge_buffer[0], fpc1080->huge_buffer[1]);	
	fpc1080->data_offset = size > FPC1080_SYNCED_REG_SIZE ?
				size + FPC1080_SYNCED_REG_SIZE : size + 1;
//	printk(KERN_ERR"*****:%s:%d write register Ended \n",__FUNCTION__,__LINE__);
	return 0;
}
...
//	printk(KERN_ERR"*****%s:%d 1:Enter \n", __FUNCTION__,__LINE__);
	...
	while (1) {
//		printk(KERN_ERR"*****%s:%d IRQ timeout value:%x 1.1:Enter \n", __FUNCTION__,__LINE__, FPC1080_DEFAULT_IRQ_TIMEOUT);	
		error = fpc1080_wait_for_irq(fpc1080,
					     FPC1080_DEFAULT_IRQ_TIMEOUT); 
/*		error = fpc1080_wait_for_irq(fpc1080,
					       15);*/
//		printk(KERN_ERR"*****%s:%d error:%d 2:Enter \n", __FUNCTION__,__LINE__,error);
		if (error == 0){
//			printk(KERN_ERR"*****%s:%d 3:Enter \n", __FUNCTION__,__LINE__);	
			break;
		}

//		printk(KERN_ERR"*****%s:%d fpc1080->thread_task.should_stop:%x4:Enter \n", __FUNCTION__,__LINE__, 
//								fpc1080->thread_task.should_stop);		
		if (fpc1080->thread_task.should_stop) {
//			printk(KERN_ERR"*****%s:%d 5:Enter \n", __FUNCTION__,__LINE__);		
			return -EINTR;
		}
		if (error != -ETIMEDOUT){
//			printk(KERN_ERR"*****%s:%d 6:Enter \n", __FUNCTION__,__LINE__);		
			return error;
		}
	}

//	printk(KERN_ERR"*****%s:%d error:%d 7:Enter \n", __FUNCTION__,__LINE__,error);
	if (error)
		return error;

	error = spi_sync(fpc1080->spi, &m);
//	printk(KERN_ERR"*****%s:%d error:%d 8:Enter \n", __FUNCTION__,__LINE__,error);
...
//	printk(KERN_ERR"*****%s:%d Addr:%x value:%x 3:Enter \n", __FUNCTION__,__LINE__, addr, value);
	spi_message_init(&m);
	spi_message_add_tail(&t, &m);
...
static int fpc1080_write_adc_setup(struct fpc1080_data *fpc1080)
{
	int error;
//	printk(KERN_ERR"*****%s:%d 1:Enter \n", __FUNCTION__,__LINE__);
//	printk(KERN_ERR"*****%s:%d value:%x 2:Enter \n", __FUNCTION__,__LINE__, fpc1080->adc_setup.gain);
...
//			printk(KERN_ERR"*****%s:%d 7:Enter \n",__FUNCTION__,__LINE__);
			fpc1080_spi_read_wait_irq(fpc1080, &irq);

		} else {
//			printk(KERN_ERR"*****%s:%d 8:Enter \n",__FUNCTION__,__LINE__);
			error = fpc1080_cmd_wait_irq(fpc1080,
...
//	printk(KERN_ERR"*****%s:%d 3:Enter \n", __FUNCTION__,__LINE__);
	error = fpc1080_start_thread(fpc1080, FPC1080_THREAD_CAPTURE_MODE);
//	printk(KERN_ERR"*****%s:%d error:%d 4:Enter \n", __FUNCTION__,__LINE__, error);
...
//	printk(KERN_ERR"*****:%s:%d counter:%d\n", __FUNCTION__,__LINE__,fpc1080->open_counter);
	if (fpc1080->open_counter > 0)
		fpc1080->open_counter--;
				...
				size_t count, loff_t *ppos)
{
	...
	avail_data = fpc1080->avail_data;
	max_dat = (count > avail_data) ? avail_data : count;
	if (max_dat) {
...
//		printk(KERN_ERR"*****%s:%d Ioctl started\n", __FUNCTION__,__LINE__);
		error = fpc1080_start_capture(fpc1080);
//		printk(KERN_ERR"*****%s:%d error:%d Ioctl end\n", __FUNCTION__,__LINE__, error);
		break;
...
//		printk(KERN_ERR"*****%s:%d Ioctl started\n", __FUNCTION__,__LINE__);
		error = fpc1080_abort_capture(fpc1080);
//		printk(KERN_ERR"*****%s:%d error:%dIoctl started\n", __FUNCTION__,__LINE__, error);	
		if (error)
			break;
//		printk(KERN_ERR"*****%s:%d Ioctl middle\n", __FUNCTION__,__LINE__);
		error = fpc1080_capture_single(fpc1080, arg);
//		printk(KERN_ERR"*****%s:%d error:%d Ioctl ended\n", __FUNCTION__,__LINE__, error);
		break;
	default:
		error = -ENOTTY;
	}
...
/*        gpio_set_value(fpc1080->reset_gpio, 0);
	...
	printk(KERN_ERR"*****%s:%d Started Hardware ID Request \n", __FUNCTION__, __LINE__);
	error = fpc1080_spi_wr_reg(fpc1080, FPC1080_REG_HW_ID, 0, 2);
	if (error)
		goto err;

//	printk(KERN_ERR"*****command:%x value:%x\n", FPC1080_REG_HW_ID, fpc1080->huge_buffer[fpc1080->data_offset]);
	if (fpc1080->huge_buffer[fpc1080->data_offset] != FPC1080_HARDWARE_ID) {
		dev_err(&fpc1080->spi->dev,
...
/*	error = fpc1080_spi_wr_reg(fpc1080,  FPC1080_THREAD_IDLE_MODE, 0, 2);
	if (error)
		goto err;
	printk(KERN_ERR"*****command:%x value:%x\n", FPC1080_THREAD_IDLE_MODE, fpc1080->huge_buffer[fpc1080->data_offset]);
*/
...
/*	bitmap_fill(fpc1080->nav_dev->keybit, KEY_MAX); */
	...
	fpc1080->adc_setup.gain = 12;
	fpc1080->adc_setup.offset = 30;
	fpc1080->adc_setup.pxl_setup = 5;
